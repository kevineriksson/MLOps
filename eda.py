...
# OOF est_duration_min for scatter plot
...
counts = df["is_high_fare"].value_counts().sort_index()
bars = plt.bar(
    ["Low Fare (0)", "High Fare (1)"],
    counts.values,
...
print(f"\n- CLASSIFICATION TARGET: is_high_fare (fare > ${fare_threshold:.2f})")
print(f"  Class 0 (low fare): {counts[0]:,} ({counts[0]/len(df)*100:.1f}%)")
print(f"  Class 1 (high fare): {counts[1]:,} ({counts[1]/len(df)*100:.1f}%)")
print(f"  The classes are imbalanced (~75/25 split)")
