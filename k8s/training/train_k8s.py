"""
Kubernetes Training Job — NYC Green Taxi Fare Prediction
Adapted from mlflow_experiments.py for K8s deployment:
- Connects to MLflow server via HTTP (not SQLite directly)
- Reads data from shared PVC at /mlflow-data/
- Saves duration model to PVC for the prediction app
"""
...
df_train_full = load_and_clean([
    os.path.join(DATA_DIR, "green_tripdata_2021-01.parquet"),
    os.path.join(DATA_DIR, "green_tripdata_2021-02.parquet"),
])
print(f"Training data size: {len(df_train_full):,} rows")

print("Loading March 2021 test data...")
df_mar = load_and_clean([os.path.join(DATA_DIR, "green_tripdata_2021-03.parquet")])
...
# Save duration model to shared PVC for the prediction app
joblib.dump(duration_model, os.path.join(DATA_DIR, "duration_model.pkl"))
print(f"  Duration model saved to {DATA_DIR}/duration_model.pkl")
...
print(f"  {'MAE':<8} {logged_mae:>12.4f} {original_metrics['mae']:>12.4f} "
      f"{'Y' if abs(logged_mae - original_metrics['mae']) < 1e-4 else 'N':>8}")
print(f"  {'RMSE':<8} {logged_rmse:>12.4f} {original_metrics['rmse']:>12.4f} "
      f"{'Y' if abs(logged_rmse - original_metrics['rmse']) < 1e-4 else 'N':>8}")
print(f"  {'R2':<8} {logged_r2:>12.4f} {original_metrics['r2']:>12.4f} "
      f"{'Y' if abs(logged_r2 - original_metrics['r2']) < 1e-4 else 'N':>8}")
